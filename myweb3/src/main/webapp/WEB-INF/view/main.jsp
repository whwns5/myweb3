...
<script>
function goUpdate(){
	var cks = $('input[name="mater_ck"]:checked');
	var update_idx = $(cks).eq(0).val();
	location.href = 'mateUpdate.do?user_no=' + update_idx;
}
function goInsert(){
	location.href = 'mateInsert.do';
}
</script>
</head>
<body>
	...
	<div class="box_right">
		...
		<div class="main_box clear_b">
			<div class="main_tab main_top">
				<img alt="all_icon" src="image/all_icon.gif"> <a href="javascript:goInsert()">입력</a>
				<img alt="all_icon" src="image/all_icon.gif"> <a href="javascript:goUpdate()">수정</a> 
			...
			<table>
				...
				<c:forEach var="dto" items="${arry_mater}">
					...
					<tr>
						<td><input type="checkbox" name="mater_ck" value="${dto.user_no}"></td>
						<td>${dto.kor_name}</td>
						<td>${dto.jumin_no}</td>
						<td>
